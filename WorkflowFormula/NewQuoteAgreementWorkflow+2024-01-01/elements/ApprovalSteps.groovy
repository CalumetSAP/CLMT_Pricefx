...
def columnNames = ["Approver", "Material", "Ship-to", "Plant", "Sales Person",
                   "Recommended Price", "Price", "Material Margin", "Material Margin %", "Index Number 1",
...
def lineItems = []
def approver, discountApprover, formulaApprover, row, configurator, numberOfDecimals, recommendedPrice, price, cost,
        materialMargin, materialMarginPercent, indexNumbers, adderValue, referencePeriodId, freightAmount, deliveredPrice,
        uom, conversionFactor, convertedPrice
...
def SoldToNumber = quote?.inputs?.find { it.name == "InputsConfigurator"}?.value?.SoldToInput
def SoldToName = quote?.inputs?.find { it.name == "InputsConfigurator"}?.value?.SoldToHiddenInput

def headerDescription = quote?.headerText

def map = [
        "soldTo"           : String.join(" - ", SoldToNumber, SoldToName),
...
def findUOMConversionTable(List lineItems) {
    def skus = quote.lineItems?.collect { it.sku }
    if (!skus) return [:]
