def conditionRecordNo = out.LoadZBPL?.ConditionRecordNo
...
if (conditionRecordNo) {
    def condRec = conditionRecordNo instanceof List ? conditionRecordNo?.find() : conditionRecordNo
    return api.global.ZBPLScales?.get(condRec) ?: []
} else {
    return []
}