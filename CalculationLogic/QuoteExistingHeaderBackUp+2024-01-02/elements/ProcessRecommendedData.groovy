...
def configurator, productMasterItem, numberOfDecimals, priceType, industry, recommendedValues, approver, formulaApprover, dsData, filteredApproversMap, approvalRequired
for (lnProduct in quoteProcessor.getQuoteView().lineItems) {
    ...
    if (approvalRequired) {
        ...
        if (priceType == "1") {
            findApproverInTable = calculations.getFormulaApproverValue(filteredApproversMap["2"], industry, productMasterItem?.PH2Code, productMasterItem?.PH1Code)
