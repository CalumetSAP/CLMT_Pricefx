...
for (lnProduct in quoteProcessor.getQuoteView().lineItems) {
    ...
    freightData = freightValues?.get(dsData.SAPContractNumber + "|" + dsData.SAPLineId)
    ...
    if (api.local.priceHasChanged?.get(lnProduct.lineId) && priceType != "3") {
    ...
    } else if (api.local.deliveredPriceHasChanged?.get(lnProduct.lineId) && priceType != "3") {
