...
def existingData = api.local.existingFreight

def conditionRecordSetMap = out.LoadConditionRecordSetMap

def newItems = api.local.newFreightItems
if (!newItems) return
...
api.local.freightItemsToUpdate = itemsToUpdateAndDelete.itemsToUpdate
api.local.freightSupersededItems = itemsToUpdateAndDelete.supersededItems
def delayedItems = itemsToUpdateAndDelete.delayedItems
api.local.freightDelayedItems = delayedItems
api.local.newFreightItems.removeAll(delayedItems)
...
def getItemsToUpdateAndDelete (newItems, existingCRs, groupByKeys, existingData, table, sdf, dateTimeSDF, shouldUpdateExistingRecords) {
    ...
    def delayedItems = []
    def conditionRecordSetMap = out.LoadConditionRecordSetMap
    def newItemValidFrom, newItemValidTo, possibleUpdatableItems, oldItemValidFrom, oldItemValidTo, key
        ...
        if (possibleUpdatableItems) {
            for (possibleUpdatableItem in possibleUpdatableItems) {
                ...
                if (newItemValidFrom <= oldItemValidFrom && newItemValidTo >= oldItemValidFrom) { //Scenario 1
                    ...
                    } else if (oldItemValidTo < newItemValidFrom-1) { //Scenario 3 - CAL-701
