...
api.logInfo("Suganya Testing here in SAP Quotes to quotes DDL")
api.log("DEBUG", "Raw JSON", jsonString)
...
if (processingSAPUpdateRows) {
   ...
   def dm = ctx.getDataSource("SAPQuotes")
    def lineIds, query
    processingSAPUpdateRows.quoteId.each {
        quoteId ->
...
def runCalculationFlow (cfName) {
    ...
    if (cf) {
        ...
        try {
            api.logInfo("Suganya Testing here SAP Quotes to Quotes DDL")
            api.logInfo("requestBody",requestBody)
