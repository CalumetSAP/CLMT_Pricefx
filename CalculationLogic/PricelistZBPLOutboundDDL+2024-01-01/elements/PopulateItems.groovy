...
Map plItems = libs.PricelistLib.Common.getAllPLItems(plId)
        ?.findAll { (it["Pricelist Number"] && it["New Effective Date"] && (it["Jobber / Dealer %"] || it["SRP %"] || it["MAP %"])) || moqHasChanged(it, api.global.pricingMap as Map) }
...
Map updateCommonFields (itemToUpdate, plItem, String plId, roundingUtils) {
    ...
    itemToUpdate.UOM = plItem["ZBPL UOM"]
    ...
    itemToUpdate.MOQ = plItem["MOQ"] && plItem["MOQ UOM"] ? plItem["MOQ"] : null
    itemToUpdate.MOQUOM = plItem["MOQ"] && plItem["MOQ UOM"] ? plItem["MOQ UOM"] : null

    return itemToUpdate
}

def moqHasChanged(row, Map pricingMap) {
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd")
    def newEffectiveDate = row["New Effective Date"] ? sdf.parse(row["New Effective Date"]) : null
    def material = row["sku"]
    def pricelist = row["key2"]?.split('-')?.getAt(1)

    def pricingData = pricingMap?.get([material, pricelist])?.find {
        it.EffectiveDate <= newEffectiveDate && it.ExpirationDate >= newEffectiveDate
    } ?: [:]

    if (!pricingData && row["MOQ"] && row["MOQ UOM"]) return true
    if (!pricingData) return false

    def previousMOQ = pricingData?.MOQ?.toBigDecimal() ?: BigDecimal.ZERO
    def previousMOQUOM = pricingData?.MOQUOM

    def currentMOQ = row["MOQ"]?.toBigDecimal() ?: BigDecimal.ZERO
    def currentMOQUOM = row["MOQ UOM"]

    return previousMOQ != currentMOQ || previousMOQUOM != currentMOQUOM
}