...
def value = out.FindShipToData && !api.isInputGenerationExecution() ? out.FindShipToData?.State : null
def entry = null
if (havePermissions && shouldShow) {
    input = libs.BdpLib.UserInputs.createInputString(
            lineItemConstants.SHIP_TO_STATE_ID,
            lineItemConstants.SHIP_TO_STATE_LABEL,
            false,
            true,
            value as String
    ).getFirstInput()
