...
def value = out.FindShipToData && !api.isInputGenerationExecution() ? out.FindShipToData?.Zip : null
def entry = null
if (havePermissions && shouldShow) {
    input = libs.BdpLib.UserInputs.createInputString(
            lineItemConstants.SHIP_TO_ZIP_ID,
            lineItemConstants.SHIP_TO_ZIP_LABEL,
            false,
            true,
            value as String
    ).getFirstInput()
