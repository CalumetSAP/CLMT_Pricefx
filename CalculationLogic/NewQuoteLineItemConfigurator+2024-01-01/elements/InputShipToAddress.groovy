...
def value = out.FindShipToData && !api.isInputGenerationExecution() ? out.FindShipToData?.Address : null
def entry = null
if (havePermissions && shouldShow) {
    input = libs.BdpLib.UserInputs.createInputString(
            lineItemConstants.SHIP_TO_ADDRESS_ID,
            lineItemConstants.SHIP_TO_ADDRESS_LABEL,
            false,
            true,
            value as String
    ).getFirstInput()
