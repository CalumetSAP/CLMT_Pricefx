...
if (((priceCompleted == lineItemConstants.PRICE_COMPLETED_PRICE_ID || priceCompleted == lineItemConstants.PRICE_COMPLETED_ADDER_ID) && (api.local.priceHasChanged || api.local.freightAmountHasChanged || api.local.numberOfDecimalsHasChanged) && priceType != "3")) {
...
if (priceCompleted == lineItemConstants.PRICE_COMPLETED_DELIVERED_PRICE_ID && (api.local.deliveredPriceHasChanged || api.local.freightAmountHasChanged || api.local.numberOfDecimalsHasChanged) && priceType != "3") {
    def numberOfDecimals = InputNumberOfDecimals?.input?.getValue() ?: "2"
...
if (api.local.priceTypeHasChanged) {
