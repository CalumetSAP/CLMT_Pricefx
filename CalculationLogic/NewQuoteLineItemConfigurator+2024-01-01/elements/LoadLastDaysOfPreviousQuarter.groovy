...
//
//if (priceType != "1" || !(api.local.indexHasChanged || api.local.priceHasChanged || api.local.adderHasChanged || api.local.freightAmountHasChanged) || !indexValues || lastDaysOfPreviousMonthReferencePeriod != "6") return
//
...
//]
//filters.addAll(indexLib.getPreviousQuarterAverageFilters(calculationDate))
//
//api.local.lastDaysOfPreviousQuarter = api.findLookupTableValues("IndexValues", ["key1", "key2", "key3", "attribute1", "attribute2", "attribute4", "attribute5"], "-key3", *filters)?.groupBy {
...
//
//return [
//        api.local.lastDaysOfPreviousQuarter[api.local.index1],
//        api.local.lastDaysOfPreviousQuarter[api.local.index2],
//        api.local.lastDaysOfPreviousQuarter[api.local.index3],
//]