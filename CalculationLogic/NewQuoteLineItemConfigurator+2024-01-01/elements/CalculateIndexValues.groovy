...
def indexOnePercent = indexValues?.size() == 1 ? 1.toBigDecimal() : (indexValues?.size() == 2 ? 0.5.toBigDecimal() : (indexValues?.size() == 3 ? (1/3).toBigDecimal() : BigDecimal.ZERO))
def indexTwoPercent = indexValues?.size() == 1 ? 0.toBigDecimal() : (indexValues?.size() == 2 ? 0.5.toBigDecimal() : (indexValues?.size() == 3 ? (1/3).toBigDecimal() : BigDecimal.ZERO))
def indexThreePercent = indexValues?.size() == 1 ? 0.toBigDecimal() : (indexValues?.size() == 2 ? 0.toBigDecimal() : (indexValues?.size() == 3 ? (1/3).toBigDecimal() : BigDecimal.ZERO))
...
if ((priceCompleted == lineItemConstants.PRICE_COMPLETED_PRICE_ID || priceCompleted == lineItemConstants.PRICE_COMPLETED_DELIVERED_PRICE_ID) &&
        (api.local.priceHasChanged || api.local.deliveredPriceHasChanged || api.local.freightAmountHasChanged || api.local.indexHasChanged || api.local.numberOfDecimalsHasChanged || api.local.recalculationDateHasChanged || api.local.priceValidFromHasChanged)) {
...
if (priceCompleted == lineItemConstants.PRICE_COMPLETED_ADDER_ID && (api.local.adderHasChanged || api.local.indexHasChanged || api.local.freightAmountHasChanged || api.local.numberOfDecimalsHasChanged || api.local.recalculationDateHasChanged || api.local.priceValidFromHasChanged)) {
