...
def dashboard = api.isDebugMode() ? "SPS" : calcItem?.Value?.dashboard

if (dashboard != "SPS") return

final dashboardQuery = libs.DashboardLibrary.Query

def quotes = out.LoadQuotes

List affectedVariants = dashboardQuery.findPendingAffectedVariantsByDashboard("SPS") ?: []
...
def pricingDate, contracts, contractLines, materials, division, salesOrgs, soldTos, shipTos, pricelists
