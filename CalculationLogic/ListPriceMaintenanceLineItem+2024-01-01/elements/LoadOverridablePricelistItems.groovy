...
if (api.global.isFirstRow && pricelistId) {
    ...
    while (someItems = api.namedEntities(api.find("XPLI", start, max,null, Filter.equal("pricelistId", pricelistId)))) {
        for (item in someItems) {
            ...
            newListPriceZLIS = manualOverrides?.find { key, value -> value.elementName == "NewListPrice" }
            //If value 'New List Price (ZLIS)' was overwritten then it applies
            if (newListPriceZLIS) {
                items.add(buildOverriddenItem([material], newListPriceZLIS?.value?.value?.toBigDecimal(), item.NewListPriceModifiedDatetime, formatter))
            }
    ...
    api.global.newListPriceZLISItems = items.groupBy { it.key }
...
api.local.newListPriceZLISOverride = api.global.newListPriceZLISItems?.get([material])?.sort { it.modifiedDateTime }?.last()
