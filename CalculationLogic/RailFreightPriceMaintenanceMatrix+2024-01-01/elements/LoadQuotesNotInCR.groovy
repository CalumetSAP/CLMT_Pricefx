...
if (libs.SharedLib.BatchUtils.isNewBatch()) {
    def combinationKeys = api.global.contractCombinationKeys as List ?: []
    def conditionRecordsFound = api.global.conditionRecordsFound as List ?: []
    ...
    api.global.activeQuotesNotInCR = evalRows(activeTempRows, effectiveDate, api.global.modeOfTransportation, api.global.meansOfTransportation as List)
    api.global.futureQuotesNotInCR = evalRows(futureTempRows, effectiveDate, api.global.modeOfTransportation, api.global.meansOfTransportation as List)
}

def activeRow = api.global.activeQuotesNotInCR?.get(api.local.sku) ?: []
def futureRow = api.global.futureQuotesNotInCR?.get(api.local.sku) ?: []

return activeRow + futureRow
