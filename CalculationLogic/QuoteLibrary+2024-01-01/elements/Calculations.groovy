...
def calculateGuardrailValues(guardrailMap, modeOfTransportation, industry, material, phs, price, uomConversionMap, pricingUOM) {
    ...
    def approvalLevels = getApprovalLevels(price, guardrailPrice, guardrailValues.attribute3, guardrailValues.attribute4, guardrailValues.attribute5)
...
def getApprovalLevels(price, guardrailPrice, pricingApprovalLevel1, pricingApprovalLevel2, pricingApprovalLevel3) {
    if (!price || !guardrailPrice) return 0

    if (price >= guardrailPrice) return 0

    def discount = BigDecimal.ONE - (price?.toBigDecimal() / guardrailPrice?.toBigDecimal())
    discount = libs.QuoteLibrary.RoundingUtils.round(discount, 4).abs() * 100
...
def noGuardrailOrApproverFound(group, subject, text) {
    def userEmails = api.find("U", 0, api.getMaxFindResultsLimit(), null, null, Filter.isNotEmpty("groups"))
            .findAll { it.groups
