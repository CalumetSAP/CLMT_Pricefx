...
if (api.global.isFirstRow || index1Override || index2Override || index3Override) {
    ...
    String firstDaysOfPreviousMonthReferencePeriod
    if (api.getManualOverride("ReferencePeriod")) {
        firstDaysOfPreviousMonthReferencePeriod = out.LoadQuotes?.ReferencePeriod
    } else {
        firstDaysOfPreviousMonthReferencePeriod = "11"
    }

    Set<String> indexs = api.global.quotes
            ?.findAll { it.ReferencePeriod == firstDaysOfPreviousMonthReferencePeriod }
    ...
    filters.addAll(indexLib.getFirstDayMonthFilter(calculationDate))

    api.global.firstDaysOfPreviousMonth = api.findLookupTableValues("IndexValues", ["key1", "key2", "key3", "attribute1", "attribute2", "attribute4", "attribute5"], "-key3", *filters)?.groupBy {
...
return [
        api.global.firstDaysOfPreviousMonth[api.local.index1],
        api.global.firstDaysOfPreviousMonth[api.local.index2],
        api.global.firstDaysOfPreviousMonth[api.local.index3],
]