...
if (api.global.isFirstRow || index1Override || index2Override || index3Override) {
    ...
    String lastDaysOfPreviousMonthReferencePeriod
    if (api.getManualOverride("ReferencePeriod")) {
        lastDaysOfPreviousMonthReferencePeriod = out.LoadQuotes?.ReferencePeriod
    } else {
        lastDaysOfPreviousMonthReferencePeriod = "5"
    }

    Set<String> indexs = api.global.quotes
            ?.findAll { it.ReferencePeriod == lastDaysOfPreviousMonthReferencePeriod }
    ...
    filters.addAll(indexLib.getLastDayMonthFilter(calculationDate))

    api.global.lastDaysOfPreviousMonth = api.findLookupTableValues("IndexValues", ["key1", "key2", "key3", "attribute1", "attribute2", "attribute4", "attribute5"], "-key3", *filters)?.groupBy {
...
return [
        api.global.lastDaysOfPreviousMonth[api.local.index1],
        api.global.lastDaysOfPreviousMonth[api.local.index2],
        api.global.lastDaysOfPreviousMonth[api.local.index3],
]