...
def soldToNames = out.FindSoldToNames && !api.isInputGenerationExecution() ? out.FindSoldToNames as Map : [:]

def labels = contractMap?.collectEntries { key, value ->
    def sapContract = key ? key.split("\\|").getAt(0).trim() : ""
    def labelKey = key ?: ""
    def soldToName = soldToNames?.get(value.SoldTo)
    def namePart = soldToName || value.ShiptoName ? soldToName + " - " + value.ShiptoName : ""
