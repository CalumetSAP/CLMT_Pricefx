...
if (api.isDebugMode()) {
    plId = "1293"
    plType = pricelistConstants.PRICE_LIST_ZBPL_PL_TYPE
} else {
...
//api.trace("loader", loader.findAll { it.SAPContractNumber == "40021638" })
...
def getFiltersForPLsWithContract (plItems) {
    ...
    def keyFilters = [:]
    if (quoteIdKeys) {
        keyFilters["QuoteID"] = quoteIdKeys
    }
    if (lineIDKeys) {
        keyFilters["LineID"] = lineIDKeys
    }
    if (updatedByIDKeys) {
        keyFilters["UpdatedbyID"] = updatedByIDKeys
    }

    def plsSAPContractNumbers = plItems?.Contract?.unique() ?: []

    return [
            SAPContractNumber: plsSAPContractNumbers,
            KeyFilters       : keyFilters
    ]
}

def getQuoteDSItems (Map filters) {
    ...
    postFiltered = postFiltered.findAll { !it.RejectionFlag }
    if (filters.SAPContractNumber) {
        if (filters.KeyFilters) {
            postFiltered = postFiltered.findAll { row ->
                boolean matchesContract = row.SAPContractNumber in filters.SAPContractNumber

                boolean matchesKeyFilters = true
                if (filters.KeyFilters.QuoteID) {
                    matchesKeyFilters = matchesKeyFilters && (row.QuoteID in filters.KeyFilters.QuoteID)
                }
                if (filters.KeyFilters.LineID) {
                    matchesKeyFilters = matchesKeyFilters && (row.LineID in filters.KeyFilters.LineID)
                }
                if (filters.KeyFilters.UpdatedbyID) {
                    matchesKeyFilters = matchesKeyFilters && (row.UpdatedbyID in filters.KeyFilters.UpdatedbyID)
                }

                matchesContract || matchesKeyFilters
            }
        } else {
            postFiltered = postFiltered.findAll { it.SAPContractNumber in filters.SAPContractNumber }
        }
    } else {
        if (filters.PriceType) postFiltered = postFiltered.findAll { it.PriceType == filters.PriceType }
        if (filters.SalesOrg) postFiltered = postFiltered.findAll { it.SalesOrg in filters.SalesOrg }
        if (filters.Material) postFiltered = postFiltered.findAll { it.Material in filters.Material }
        if (filters.PriceListPLT) postFiltered = postFiltered.findAll { it.PriceListPLT in filters.PriceListPLT }
    }
