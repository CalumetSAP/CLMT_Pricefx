...
def processingQuotes = libs.QuoteLibrary.Calculations.getProcessingQuotesRows()?.quoteId?.unique()
if (!processingQuotes) return
...
if (!jobId) {
    errorQuotes = []
    completedQuotes = processingQuotes
} else {
...
quoteCalculations.addOrUpdateStatusToPendingCustomEvent(completedQuotes)
quoteCalculations.addOrUpdateStatusToError(errorQuotes)