...
for (lnProduct in quoteProcessor.getQuoteView().lineItems) {
    ...
    if (priceType == "1") {
        if (adder != null && (!price || !deliveredPrice)) adderHasChanged.put(lnProduct.lineId, true)
        if (price && (!deliveredPrice || adder == null)) priceHasChanged.put(lnProduct.lineId, true)
        if (deliveredPrice && (!price || adder == null)) deliveredPriceHasChanged.put(lnProduct.lineId, true)
    }
    if (priceType == "2") {
