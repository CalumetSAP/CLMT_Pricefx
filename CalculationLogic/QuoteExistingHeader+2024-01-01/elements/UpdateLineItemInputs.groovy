...
// Import line items
if (!api.local.linesToAdd) return

Set linesToAdd = api.local.linesToAdd as Set
def contracts = out.FindContractDSData
def filteredContracts = contracts.collectEntries { contract, lines ->
    def filteredLines = lines.findAll { row ->
        linesToAdd.contains(contract + "|" + row.LineNumber)
    }
    filteredLines ? [(contract): filteredLines] : [:]
}
...
filteredContracts?.each { contractNumber, lines ->
    lines?.each { line ->
        def parameters = []
