...
for (lnProduct in quoteProcessor.getQuoteView().lineItems) {
    ...
    price = costAttrs?.StandardPrice ? costAttrs?.StandardPrice : costAttrs?.SecondayPrice
    pricingUOM = calculations.getInputValue(lnProduct, lineItemConstants.PRICING_UOM_ID) ?: productMasterItem?.UOM

    cost = calculations.calculateLineItemCost(price, costAttrs?.CostingLotSize, units?.toBigDecimal())
