String material = api.local.material
String plantCode = out.LoadQuotes?.Plant?.split(" - ")?.first()
api.local.plantCode = plantCode

// when the new batch starts, do pre-load product costs (for all SKUs of the batch) into memory
if (libs.SharedLib.BatchUtils.isNewBatch()) {
    final calculations = libs.PricelistLib.Calculations
    def filters = [
            Filter.equal("name", "Cost"),
            Filter.in("sku", api.global.currentBatch),
            Filter.isNotEmpty("attribute5"),
            Filter.isNotNull("attribute5"),
            Filter.equal("attribute6", "USD"),
            Filter.or(
                    Filter.notEqual("attribute1", "Z1"), Filter.isEmpty("attribute1")
            )
    ]
    def fields = ["sku", "attribute1", "attribute2", "attribute3", "attribute4", "attribute5", "attribute9"]
    api.global.productCosts = api.stream("PX", "sku", fields, *filters).withCloseable {
        it.collect()
    }.groupBy {
        [it.sku, it.attribute2]
    }.collectEntries {
        [(it.key): [
                costAverage         : calculations.getCostAverage(it.value),
                uom                 : it.value.first().attribute4,
                xPlantMaterialStatus: it.value.attribute1.find()
        ]]
    }
}

return api.global.productCosts?.get([material, plantCode])